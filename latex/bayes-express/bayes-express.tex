...
\begin{table}[t!]
  ...
  \begin{tabular}{|c|c|}
    ...
    \\
    \footnotesize (index) & \footnotesize (multi-index)
    \\ \hline \hline
    $\vdots$ & $\vdots$
    \\ \hline
  \end{tabular}
  ...
  \mycaption{tab:rna-seq-data}{Database schema for target-aligned RNA
    ...
    rendered as a floating-point number. In all of the tables, the
    first column serves as a unique index. In the read grouping and
    alignment tables, the sample and target may also be reverse
    indexed for more efficient access to all the reads in a sample or
    all reads aligning to a target.} \end{table}
...
In transcriptome analysis, it is not unusual to find reads that align
to more than one isoform.  Furthermore, alignment tools based on
shredding reads into $k$-mers, such as Sailfish \citep{patro2014sailfish} and
kallisto \citep{bray2016near}, return sets of targets compatible with
a read rather than a distinct alignment position.  Statistically, we
will treat the true source of a read as missing data.

\section{Cassette exon example}

Sticking to the example we were working with before, suppose we have
two isoforms, one with and one without a cassette exon, as shown in
...
simple alignment of reads to the genome and discard reads that cross
exon junctions.  All of our reads will then fall entirely wtihin exons
$A$, $B$, or $C$.  The only distinctive read for inclusion is one
falling in $B$; there are no distinctive reads for exclusion.
...
aligning to $B$.  But we can with all of the reads.  One way to get
the right number of reads is to assume the inclusion isoform
generates 30 reads from $A$, 30 from $B$, and 30 from $C$, while the
exclusion isoform generates 70 reads from $A$ and 70 from $C$.  The
proportion of reads assigned to the inclusion isoform is thus
\[
  \hat{\theta} \ = \ \frac{30 + 30 + 30}{(30 + 30 + 30) + (70 + 70)}
  \ \approx \ 0.39.
\]
If we assume the exon lengths are all the same, the inclusion form has
length $L_1 = 90$ and the exclusion form has length $L_2 = 60$.
Without adjusting length further (e.g., with hexamer binding or
positional weighting), we expect 3 inclusion reads for every 2
exclusion reads if they have the same expression level.  In general,
we know that the proportion of inclusion reads will be
\[
...
\]
and plug in $\hat{\theta}$, $L_1$, and $L_2$ to get
$\hat{\psi} = 0.30$, just as we'd expect.  All that remains is to
formalize the problem as a Bayesian posterior inference problem, in
order to derive uncertainty in our estimates.


\section{Data format}

As before, we will assume there are $T \in \mathbb{N}$ target
sequences and $N \in \mathbb{N}$ reads.  As before, we restrict
attention at first to $T = 2$ targets, such as the set of transcripts
with a cassette exon included and those where it's excluded.

Rather than requiring each read $n \in \rngto{N}$ to map to a single
target in $\rngto{T}$, our observed data will be a set of possible
targets,
\[
  y_n \subseteq \rngto{T}.
\]
Such a set may be derived from the compatiblity sets of
\cite{bray2016near}.  In the splicing-in comparison, with $T = 2$,
some reads will be compatible with inclusion and with exclusion of the
cassette exon.

We also assume generalized weights $L_t \in (0, \infty)$ for each
target $t \in T$, which will be used as in the previous chapter to
adjust the expected number of reads for a given expression level.


\section{Generative model}

The generative model for reads does not change.  Continuing with the
running example of cassette exons, we still assume
reads are generated by drawing a random sequence from mRNA distributed
according to the proportion spliced in, $\psi \in (0, 1)$, with
sequences then drawn according to their weight, which might adjust for
position, binding hexamer, etc.
...
\[
  z_n \in \rngto{T}, \ \textrm{for} \ n \in \rngto{N}.
\]
We further know that $z_n$ satisfies the constraint of being
compatible with $y_n$.  The sampling distribution for drawing a read
is
\[
  z_n \sim \textrm{Bernoulli}(\theta),
\]
where, as before, $\theta$ is the adjusted probability of reading from
sequence 1.
...
not observe $z_n \in \rngto{T}$, only $y_n \subseteq \rngto{T}$.  The
trick we will use is to define a complete data likelihood function
$p(y, z \mid \psi, L)$ and then marginalize out $z$ to derive a
likelihood
\[
  p(y \mid \psi, L) = \sum_z p(y, z \mid \psi, L).
\]
We will factor the complete data likelihood as
\[
...
\[
  \theta = \frac{\psi \, L_1}{\psi \, L_1 + (1 - \psi) \, L_2}
\]
is the weight adjusted probability of an inclusion read.

The complete data likelihood is fully defined by
assuming that the compatiblity set $y_n$ is chosen uniformly among
the finite set of possibilities including $z_n$,\footnote{For those
  who are counting, the size of this set is $2^{T - 1}$.}
\[
  y_n \sim \textrm{uniform}\!\left(\{ z_n \} \cup 2^{\rngto{T} \ \setminus \ z_n}\right),
\]
where $2^A = \{ a : a \subseteq A \}$ is the powerset operator and
$A \setminus b = \{ a : a \in A, a \neq b \}$ is the set difference
operator.

\subsection{Likelihood}

Now we turn the crank to marginalize out the missing data variables
$z$.  The first step is to localize the effect of the missing data to
single observations.  Our complete-data likelihood factors by
observation as
\[
  p(y, z \mid \psi, L)
  = \prod_{n=1}^N p(y_n, z_n \mid \psi, L).
\]
This allows us to work on marginalizing out one $z_n$ at a time, which
gives us
%
\begin{eqnarray*}
  p(y_n \mid \psi, L)
...
corresponding to the alignment in $y_n$ for read $n$, we
can perform inference on the missing values conditional on the
parameters $\psi$ by noting that for $z_n \in y_n$, we have
\[
  p(z_n \mid y_n) \propto p(z_n \mid \psi, L),
\]
which normalizes by summing over the possible alignments,
\[
  p(z_n \mid y_n)
